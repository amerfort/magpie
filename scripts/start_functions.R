...
getReportData <- function(path_to_report_bioenergy, mute_ghgprices_until = "y2010", path_to_report_ghgprices = NA) {
  ...
  .CDR_tax_factor <- function(mag){
    price_co2 <- mag[,,"Price|Carbon|Guardrail (US$2005/t CO2)"]*44/12 # US$2005/tCO2 -> US$2005/tC
    price_co2[price_co2==0] <- 1e-4

    price_CDR <- mag[,,"Price|Carbon|CDR (US$2005/t CO2)"]*44/12 # US$2005/tCO2 -> US$2005/tC
    ...
    notGLO <- getRegions(mag)[!(getRegions(mag)=="GLO")]
    # delete old input file before updating it
    f <- "./modules/56_ghg_policy/input/f56_CDR_tax_factor.cs3"
    suppressWarnings(unlink(f))
    write.magpie(out[notGLO,,],f)
  }
